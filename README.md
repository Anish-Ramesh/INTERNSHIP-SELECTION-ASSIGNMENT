...
├── agent/                  # Core Agent Logic (ReAct Loop, Prompts, Memory)
│   ├── cache/              # Persistent Trace Cache (JSON)
│   ├── agent_loop.py       # Main orchestration & state management
│   ├── agent_utils.py      # Shared logic (Deduplication, Tokenization)
│   ├── bonus_features.py   # Telemetry, Cost Tracking & Reflection
│   ├── prompts.py          # System Reasoning Protocols
├── dataset/                # Data Corpus
│   ├── movies.db           # SQLite DB (Generated by preprocess.py)
│   ├── unstructured_reviews/ # Raw .txt review documents
├── tools/                  # Modular Tool Layer
│   ├── query_data.py       # SQL Precision Tool
│   ├── search_docs.py      # BM25 Qualitative Tool
│   ├── web_search.py       # Real-time Tavily Tool
├── utils/                  # Infrastructure Utilities
│   ├── logger.py           # ReAct TraceLogger (Terminal Output)
├── DESIGN.md               # Detailed Architecture Design
├── EVALUATION.md           # 20-Question Performance Report
├── README.md               # Main Documentation
├── setup_project.py        # Automated One-Click Installation
└── .env                    # API Secrets (User Provided)
```
